...
There is no stock `section-how-to-participate.md`.  If it is to be used, it should be provided in the event directory.

#### Stay-in-touch

The stock `section-stay-in-touch.md` lists the tutorial mailing list, mentions that we'll respond to PRs and issues from the hands-on exercises, the IDEAS announcement list and the BSSw digest.

#### Requested-citation

The stock `section-requested-citation.md` will usually suffice for this.  It constructs the citation from the presenters, the event name, the venue information, and the DOI link.  It requires the following data to be set:
...
The `section-hands-on-exercises.md` file is currently specific to the event.  It is probably most convenient to copy it and the related hands-on files from a recent tutorial with a hands-on:
  * `section-hands-on-exercises.md`
  * `handson-m*.md`
  * `images/`
